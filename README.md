...
<p align="center"><strong>A secrets manager you run yourself. Your secrets never touch anyone else's cloud.</strong></p>
...
<p align="center">
  ...
  <img alt="Self-hosted" src="https://img.shields.io/badge/deploy-self--hosted-D92D20">
</p>
...
**Envi** is an environment-secret manager: encrypted `.env` values, scoped access, an audit trail, and a CLI that gets a secret from your database into a running process without anyone typing it into Slack. It's built to be hosted on infrastructure you control, not rented as someone else's SaaS.
...
- 🎚️ **Scoped access** — grant a collaborator `read`, `write`, or `manage` on a project or a single environment. Production doesn't get touched by accident.
...
- ✉️ **Email invitations** — invite a teammate by address; they click through, sign in (or sign up on the spot if they're new), and land with access already waiting.
- 🖥️ **CLI + dashboard, one API** — `envi pull`/`push`/`diff` your `.env` files from the terminal, or manage everything visually. Same backend, same permissions, your choice of interface.
...
Envi is self-hosted only — there's no managed cloud version, by design. Running it means running four things:
