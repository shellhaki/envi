...
export default function AuthForm({ next }: { next?: string }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      // here invites a second submit on a slow connection, and the code is
      // single-use, so the retry would fail with "invalid or expired".
      if (sent) { router.push(next || "/dashboard"); router.refresh(); return; }
      setSent(true); setBusy(false);
    } catch {
      // A throwing fetch (offline, DNS, timeout) would otherwise leave the
      // button stuck disabled on "Working..." with nothing explaining why.
      setError("Couldn't reach Envi. Check your connection and try again.");
      setBusy(false);
    }
  }
  return <form className="auth-form" onSubmit={submit}><label>{sent ? "One-time code" : "Email address"}<input autoFocus required type={sent ? "text" : "email"} inputMode={sent ? "numeric" : "email"} autoComplete={sent ? "one-time-code" : "email"} value={sent ? code : email} onChange={(e) => sent ? setCode(e.target.value) : setEmail(e.target.value)} placeholder={sent ? "123456" : "you@company.com"} /></label>{error && <p className="form-error">{error}</p>}<button className="button accent large" disabled={busy}>{busy ? "Working..." : sent ? "Verify and continue" : "Send login code"}{busy ? <span className="spinner" /> : <ArrowRight />}</button>{sent && <button type="button" className="link-button" onClick={() => { setSent(false); setCode(""); }}>Use another email</button>}</form>;
}
